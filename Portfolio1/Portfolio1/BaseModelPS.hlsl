...
PixelOutput main(DomainOutput input) : SV_TARGET
{
    ...
    if (bIsNormalTexture)
    {
        fNormalSampled = GetSampledNormalFromTBN(Sampler, NormalTexture, input.f2TexCoord, input.f4ProjNormal, input.f4ModelTangent, input.f4ModelBiTangent);
    }
    else
    {
        fNormalSampled = input.f4ProjNormal;
    }
    ...
    for (int i = 0; i < MAX_LIGHT_NUM; ++i)
    {
        ...
        float fLightPower = 10 * clamp(dot(tolight, fNormalSampled), 0.f, 1.f);
